...
<div class="card card-outline card-primary">
	<div class="card-header">
		...
		<div class="card-tools">
			<a href="./?page=products/manage_product" id="create_new" class="btn btn-flat btn-sm btn-info"><span class="fas fa-plus"></span> Add New</a>
		</div>
	</div>
	<div class="card-body">
			...
			<div class="container-fluid">
				<table class="table table-hover table-striped">
					...
					<tbody>
						<?php
						$i = 1;
						$brand_qry = $conn->query("SELECT id,`name` FROM `brand_list` where id in (SELECT brand_id from `car_list`)");
						$brand_res = $brand_qry->fetch_all(MYSQLI_ASSOC);
						$brand_arr = array_column($brand_res, 'name', 'id');
						$category_qry = $conn->query("SELECT id,`name` FROM `category_list` where id in (SELECT category_id from `car_list`)");
						$category_res = $category_qry->fetch_all(MYSQLI_ASSOC);
						$category_arr = array_column($category_res, 'name', 'id');
						$qry = $conn->query("SELECT * from `car_list` order by `product_title` asc");
						...
						?>
							<tr>
								...
								<td class="text-center">
									...
									switch ($row['status']) {
										case '0':
											echo "<span class='badge badge-success bg-info badge-pill'>Available</span>";
											break;
										case '1':
